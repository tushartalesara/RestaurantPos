-- Run this after:
-- 1) 017_order_billing_fields.sql
--
-- Adds:
-- - shared billing calculation helper for order writes
-- - billing-aware voice and manual order RPCs
-- - subtotal remains the menu-price subtotal
-- - tax is extracted for tax-inclusive restaurants instead of added again

drop function if exists public.calculate_order_billing(uuid, numeric, numeric);

create function public.calculate_order_billing(
...
declare
  ...
  v_config_tax_rate_override numeric(5, 2);
  v_config_tax_inclusive boolean;
  v_config_tax_label text;
  v_config_service_fee_enabled boolean;
  v_config_service_fee_type text;
  v_config_service_fee_value numeric(8, 2);
  v_config_service_fee_label text;
  v_config_tip_label text;
  v_selected_tax_rate numeric(5, 2);
  v_selected_tax_name text;
  v_default_tax_rate numeric(5, 2);
  v_default_tax_name text;
  v_effective_tax_rate numeric(5, 2);
  v_subtotal_amount numeric(12, 2) := round(greatest(coalesce(p_subtotal_amount, 0), 0)::numeric, 2);
  v_tax_amount numeric(12, 2) := 0;
  v_service_fee_amount numeric(12, 2) := 0;
  v_tip_amount numeric(12, 2) := round(greatest(coalesce(p_tip_amount, 0), 0)::numeric, 2);
  v_total_price numeric(12, 2) := 0;
begin
  select
    r.country_code,
    r.currency_code,
    rbc.tax_rate_override,
    rbc.tax_inclusive,
    rbc.tax_label,
    rbc.service_fee_enabled,
    rbc.service_fee_type,
    rbc.service_fee_value,
    rbc.service_fee_label,
    rbc.tip_label,
    ctr.rate_percent,
    ctr.tax_name
  into
    v_country_code,
    v_currency_code,
    v_config_tax_rate_override,
    v_config_tax_inclusive,
    v_config_tax_label,
    v_config_service_fee_enabled,
    v_config_service_fee_type,
    v_config_service_fee_value,
    v_config_service_fee_label,
    v_config_tip_label,
    v_selected_tax_rate,
    v_selected_tax_name
  from public.restaurants r
  left join public.restaurant_billing_config rbc
    on rbc.restaurant_id = r.id
  left join public.country_tax_rates ctr
    on ctr.id = rbc.tax_rate_id
  ...
  into
    v_default_tax_rate,
    v_default_tax_name
  from public.country_tax_rates ctr
  where ctr.country_code = coalesce(v_country_code, 'GB')
    and ctr.is_default = true
  order by ctr.effective_from desc
  limit 1;

  v_effective_tax_rate := coalesce(v_config_tax_rate_override, v_selected_tax_rate, v_default_tax_rate, 0);
  tax_inclusive := coalesce(v_config_tax_inclusive, false);
  tax_label := coalesce(nullif(trim(coalesce(v_config_tax_label, '')), ''), v_selected_tax_name, v_default_tax_name, 'VAT');
  service_fee_label := coalesce(nullif(trim(coalesce(v_config_service_fee_label, '')), ''), 'Service Charge');
  tip_label := coalesce(nullif(trim(coalesce(v_config_tip_label, '')), ''), 'Gratuity');
  currency_code := coalesce(nullif(trim(coalesce(v_currency_code, '')), ''), 'GBP');

  if v_effective_tax_rate > 0 then
    if tax_inclusive then
      v_tax_amount := round((v_subtotal_amount * v_effective_tax_rate / (100 + v_effective_tax_rate))::numeric, 2);
    else
      v_tax_amount := round((v_subtotal_amount * (v_effective_tax_rate / 100))::numeric, 2);
    end if;
  end if;

  if coalesce(v_config_service_fee_enabled, false) then
    if v_config_service_fee_type = 'flat' then
      v_service_fee_amount := round(greatest(coalesce(v_config_service_fee_value, 0), 0)::numeric, 2);
    elsif v_config_service_fee_type = 'percent' then
      v_service_fee_amount := round((v_subtotal_amount * (greatest(coalesce(v_config_service_fee_value, 0), 0) / 100))::numeric, 2);
    end if;
  end if;

  v_total_price := round((v_subtotal_amount + v_service_fee_amount + case when tax_inclusive then 0 else v_tax_amount end + v_tip_amount)::numeric, 2);

  subtotal_amount := v_subtotal_amount;
  tax_amount := v_tax_amount;
  tax_rate_percent := coalesce(v_effective_tax_rate, 0);
...
revoke all on function public.calculate_order_billing(uuid, numeric, numeric) from public;
grant execute on function public.calculate_order_billing(uuid, numeric, numeric) to authenticated;
grant execute on function public.calculate_order_billing(uuid, numeric, numeric) to service_role;

drop function if exists public.place_voice_order_atomic(uuid, text, text, text, text, jsonb);
drop function if exists public.place_voice_order_atomic(uuid, text, text, text, text, jsonb, text, text, text, text);
...
drop function if exists public.save_manual_order_atomic(uuid, uuid, text, text, text, text, jsonb, text, text, text, text);
