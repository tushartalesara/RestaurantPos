...
-- 5) 009_fast_fail_order_locks.sql
-- 6) 010_manual_order_stock_atomic.sql
--
-- Adds:
-- - pickup vs delivery storage on restaurant_orders
-- - delivery postcode + selected delivery address storage
-- - pickup unpaid / delivery COD payment collection storage
-- - updated voice + manual order RPCs that validate and persist fulfilment data

alter table public.restaurant_orders
  add column if not exists fulfillment_type text,
  add column if not exists delivery_postcode text,
  add column if not exists delivery_address text,
  add column if not exists payment_collection text;

update public.restaurant_orders
set fulfillment_type = case
  when lower(coalesce(fulfillment_type, '')) = 'delivery' then 'delivery'
  else 'pickup'
end
where fulfillment_type is null
  or lower(coalesce(fulfillment_type, '')) not in ('pickup', 'delivery');

update public.restaurant_orders
set payment_collection = case
  when lower(coalesce(fulfillment_type, 'pickup')) = 'delivery' then 'cod'
  else 'unpaid'
end
where payment_collection is null
  or lower(coalesce(payment_collection, '')) not in ('unpaid', 'cod');

alter table public.restaurant_orders
  alter column fulfillment_type set default 'pickup',
  alter column fulfillment_type set not null,
  alter column payment_collection set default 'unpaid',
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'restaurant_orders_fulfillment_type_check'
  ) then
    alter table public.restaurant_orders
      add constraint restaurant_orders_fulfillment_type_check
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'restaurant_orders_payment_collection_check'
  ) then
    alter table public.restaurant_orders
      add constraint restaurant_orders_payment_collection_check
      check (payment_collection in ('unpaid', 'cod'));
  end if;
end $$;

create index if not exists idx_restaurant_orders_fulfillment_status
  on public.restaurant_orders(restaurant_id, fulfillment_type, status);
