...
function toRequestedQuantity(value: unknown): number {
...
async function resolveMenuItem(params: {
  ...
  if (params.itemId) {
    const byId = await params.supabase
      .from("menu_items")
      .select("id, restaurant_id, name, stock_quantity, is_available, base_price")
...
serve(async (request) => {
  ...
  const requestedQty = toRequestedQuantity(bodyObj.requested_qty)
  ...
  try {
    ...
    return jsonResponse(200, {
      ...
      note: "Use place-order-atomic tool to commit order with transactional stock enforcement.",
    })
  } catch (error) {
